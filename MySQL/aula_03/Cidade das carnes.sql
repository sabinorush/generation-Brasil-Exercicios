...
select * from tb_produtos order by nome, preco;/* Consulta os itens por nome e preço */ 

select * from tb_produtos where preco > 20.00 and categoria_id = 1; /* consulta os itens que sejam mais que 20 reais e da categoria 1*/ 

select * from tb_produtos where preco > 20.00 or categoria_id = 1;

select * from tb_produtos where not categoria_id = 1; /* trás todos os itens que não sejam 1 */
...
select * from tb_produtos where dtvalidade between "2021-11-7" and "2021-11-15";

select count(*) from tb_produtos; /* Seleciona todos as linhas que tem na tabela*/
